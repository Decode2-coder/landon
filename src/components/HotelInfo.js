...
const HotelInfo = () => {
  ...
  const loadAccessibilityData = async () => {
    try {
      const resp = await fetch(
        "https://14u770ll0i.execute-api.us-east-1.amazonaws.com/Production/accessibilities"
      );
  ...
  const loadServicesData = async () => {
    try {
      const resp = await fetch(
        "https://14u770ll0i.execute-api.us-east-1.amazonaws.com/Production/services"
      );
      const jsonData = await resp.json();
      ...
      <article id="usefulinfo">
        <section id="arrivalinfo">
          ...
          <ul>
            ...
            <li>
              <strong>Parking:</strong> Self-parking in the underground garage is ￡15 per day and valet-parking is ￡50 per day.
            </li>
            <li>
              <strong>Airport Shuttle:</strong> Our complimentary airport shuttles leave every hour on the hour, and make trips to Heathrow and Gatwick airports.
            </li>
            <li>
              <strong>Trains:</strong> The nearest Underground station is at Leicester Square.
            </li>
            <li>
              <strong>Pet Policy:</strong> Pets of all sizes and types are allowed in designated pet rooms, and the specified common areas. Service animals are allowed everywhere.
            </li>
          </ul>
        </section>
        ...
        <section className="checklist" id="accessibility">
          ...
          <ul>
            {accessibilityData.map((accessibility) => (
              <li key={accessibility.id}>{accessibility.name}</li>
            ))}
          </ul>
        </section>
      </article>
